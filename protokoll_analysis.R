...
my_stopwords = cat(stopwords(language = "de"), file = "resources/german.txt", sep = "\n")

stopwords.de = stopwords::stopwords("de")
stopwords.en = stopwords::stopwords("en")
stopwords.new = readLines("resources/stopwords-de.stopwords")
stopwords.all = c(stopwords.de, stopwords.en, stopwords.new)
view(stopwords.all)
### Define variables
...
len_split <- 500 # length of the split texts (they will be the actual documents to work on)
...
file_list <- list.files("konvention", full.names = T)
file_list
...
for(i in 1:length(file_list)){
  ...
  tm_corpus[[i]] <- tm_corpus[[i]][which(str_length(tm_corpus[[i]]) > 3)]
  ...
  names(tm_corpus)[i] <- gsub(pattern = "konvention/|.txt", replacement = "", x = file_list[i])
...
for(i in 1:length(tm_corpus)){
  ...
  # use total length to get the number of konvention you can split it
...
text.instances <- 
  mallet.import(text.array = my_texts, 
                stoplist = stopwords.all, # this removes the stopwords (ie. function words) 
...
for(i in 1:num_topics){
  words.per.topic <- mallet.top.words(topic.model, word.weights = topic.words[i,], num.top.words = 30)
...
library(wordcloud)
#png(filename = "topic_clouds.png", width = 1500, height = 1500)
pdf("word-clouds.pdf")
par(mar=c(2,2,2,2))
for(i in 1:num_topics)
{
  id = (i-1)*30 + 1
  words <- top_words$term[id:(id+29)]
  # extract the probabilites of each of the 40 terms
  probabilities <- top_words$weight[id:(id+29)]
...
# option 1. you can downloda the files and simply add them to the "corpus" directory in this project
